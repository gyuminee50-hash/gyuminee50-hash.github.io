...
def run_regime():
    ...
    def _dot(s): return '🟢' if s > 0 else ('🔴' if s < 0 else '⚪')

    now_str  = datetime.now().strftime('%m/%d %H:%M')
    score_line = f'{_dot(s_vix)}VIX{s_vix:+d} | {_dot(s_rate)}금리{s_rate:+d} | {_dot(s_spy)}SPY{s_spy:+d} | {_dot(s_dxy)}DXY{s_dxy:+d}'

    msg = '\n'.join([
        f'<b>{icon} {regime}  {total:+d}점  |  {now_str}</b>',
        score_line,
        '',
        f'<b>💼 {alloc}</b>',
        '',
